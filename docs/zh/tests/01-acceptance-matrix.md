...
| ChatPanel | 可收起/展开；session 列表和 messages 列表 UI 可切换；输入区可发送；sessions view 发送创建新 session；messages view 发送续写当前 session；发送后立即进入 messages view；`clientMessageId` 去重 |
...
不允许只以“目录已经创建”作为完成。当前版本必须满足：`ControlPanel` 和 `StageView` 可用，`ChatPanel` 能通过本地 Server 创建/续写 session，资源真源在 `assets/`，旧 Viewer 入口和旧 runtime manifest 不再使用。
